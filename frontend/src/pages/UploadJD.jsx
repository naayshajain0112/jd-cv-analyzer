...
import { extractJDText, extractJDFile, getSavedJDs, getJD, deleteJD } from '../api';
...
export default function UploadJD() {
  ...
  const handleUseSelectedJD = async () => {
    ...
    try {
      const data = await getJD(selectedJD);
  ...
  try {
    const data = await deleteJD(selectedJD);
