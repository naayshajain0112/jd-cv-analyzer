...
import { getAssessment, getJD, overrideAssessment, getReportPDFUrl, fetchSummaryPDFBlob } from '../api';
...
function CandidateCard({ assessment, jd, onOverrideSaved }) {
  ...
  // Prefer the backend's skill-group-aware gap analysis (it knows about
  // ANY_ONE skill groups). Fall back to a naive per-skill computation only
  // for older assessments created before this field existed.
  const gaps = assessment.skillGapAnalysis?.gaps
    ?? [...hardRatings, ...softRatings].filter((r) => r.rating <= 1).map((r) => r.skill);
  const weak = assessment.skillGapAnalysis?.weak
    ?? [...hardRatings, ...softRatings].filter((r) => r.rating === 2).map((r) => r.skill);
...
export default function Report() {
  ...
  const handleDownloadSummaryPDF = async () => {
    ...
    try {
      const blob = await fetchSummaryPDFBlob(jdId, assessmentIds);
      const url = window.URL.createObjectURL(blob);
      const link = document.createElement('a');
      link.href = url;
      link.download = `${sanitizeFilename(jd?.extracted?.jobTitle || jd?.title || 'Assessment')}_Candidates_Summary.pdf`;
      document.body.appendChild(link);
      link.click();
      link.remove();
      window.URL.revokeObjectURL(url);
    } catch (e) {
      alert(e.message || 'Failed to download summary PDF.');
    } finally {
  ...
  return (
    <div className="page">
      <div className="container">
        ...
        {/* All-candidates summary (this assessment run only) */}
        <div className="glass-card report__all-summary">
          <div className="report__all-summary-header">
            <h3 className="report__all-summary-title">📋 All Candidates Summary</h3>
            <button
          ...
          <div className="report__all-summary-table-wrap">
            <table className="report__all-summary-table">
              <thead>
                <tr>
                  <th>#</th>
                  <th>Candidate</th>
              ...
              <tbody>
                {sorted.map((a, i) => {
                  ...
                  const finalScore   = getEffectiveScore(a);
                  const finalVerdict = override?.overrideVerdict ?? scoring.classification ?? 'N/A';
                  const eligible = a.eligibility?.overallEligible;
                  return (
                    <tr key={a._id}>
                      <td>{i + 1}</td>
                      <td>{a.candidateName}</td>
                      <td style={{ fontWeight: 700, color: scoreColor(finalScore) }}>{finalScore}%</td>
                      <td><span className={`badge ${verdictClass(finalVerdict)}`}>{finalVerdict}</span></td>
                      <td>
                        <span className={`badge ${eligible ? 'badge-success' : 'badge-danger'}`}>
                          {eligible ? '✓ Eligible' : '✗ Not Eligible'}
                        </span>
                      </td>
                      <td>{scoring.hardSkillScore ?? 0}%</td>
                      <td>{scoring.softSkillScore ?? 0}%</td>
                    </tr>
                  );
                })}
              </tbody>
