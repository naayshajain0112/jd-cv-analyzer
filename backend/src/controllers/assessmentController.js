...
const assessCandidates = asyncHandler(async (req, res) => {
  ...
  for (const file of req.files) {
    try {
      ...
      // Identify gaps for interview questions (skill-group aware — an ANY_ONE
      // group that's satisfied by one skill should not flag its other members)
      const { gaps, weak } = identifyGaps(hardRatings, softRatings, jd.extracted?.skillGroups || []);
